---
title: "AC-DC Power Module"
description: "OEM AC-DC power modules from China: 5W–250W, IEC 62368-1/UL certified, universal input 85–264VAC. Factory audit and inspection included."
canonical_url: "https://china-sourcing-agents.com/products/ac-dc-power-module/"
---

# AC-DC Power Module

> **Read the full guide**: [https://china-sourcing-agents.com/products/ac-dc-power-module/](https://china-sourcing-agents.com/products/ac-dc-power-module/)

OEM AC-DC power modules from China: 5W–250W, IEC 62368-1/UL certified, universal input 85–264VAC. Factory audit and inspection included.

---

AC-DC power modules are one of the most compliance-critical components in any electronics product. Get the wrong grade from the wrong factory, and you are looking at failed certification tests, costly re-spins, or — in the EU — customs holds. This page covers what separates competent Chinese suppliers from budget-tier gambles, and where the real compliance traps are for Western buyers.

IEC 60950-1 (the old IT equipment safety standard) was withdrawn in December 2020 for new product submissions in most markets. The replacement is IEC 62368-1, a hazard-based safety engineering (HBSE) standard that covers audio/video, IT, and communications equipment under one framework.

The practical difference for power module buyers: **a CB test report citing IEC 60950-1 is no longer valid for new CE or UL submissions**. Yet a meaningful share of Chinese power module inventory — especially from smaller Shenzhen manufacturers running legacy tooling — still carries IEC 60950-1 certificates issued before the cutoff. They are legally grandfathered for existing products in some markets but cannot be used to certify a new product for EU or US sale.

---

## What this covers

- IEC 62368-1 vs IEC 60950-1: Why the Safety Standard Transition Still Catches Buyers Out
- Standby Power and Efficiency Regulations: EU ErP Lot 6 and US DOE Level VI
- Leakage Current and Safety Earth: Class I vs Class II for Your Application
- Chinese Supplier Landscape: Tier Structure and What to Actually Check

---

## Further reading

- [factory audit](https://china-sourcing-agents.com/services/factory-audit/)
- [power electronics sourcing overview](https://china-sourcing-agents.com/industries/power-electronics/)
- [sourcing power modules](https://china-sourcing-agents.com/services/sourcing/)
- [pre-production inspection](https://china-sourcing-agents.com/services/inspection/)
- [Full guide on China Sourcing Agent](https://china-sourcing-agents.com/products/ac-dc-power-module/)

