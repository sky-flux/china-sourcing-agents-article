...
**Speed-separation monitoring (SSM) is not built into the robot.** SSM — where the robot reduces speed as a human enters a monitored zone — requires a validated external safety sensor: laser scanner (SICK S3000, Pilz PSENscan), camera-based system (Pilz PITreader), or lidar. The cobot itself does not provide SSM. If a Chinese supplier quotes "SSM capable" as a robot feature, ask specifically whether a safety-rated sensor is included and which EN/IEC standard it is certified to. Usually it is not included.
